...
adding channel as a grouping feature for the next_click and rewriting the set-up
"""
...
def DO(frm,to,fileno):
    ...
    for i in range(0,naddfeat):
        if i==0: selcols=['ip', 'channel']; QQ=4;  #'ip', 'channel'
        if i==1: selcols=['ip', 'device', 'os', 'app']; QQ=5; #'ip', 'device', 'os', 'app'
        if i==2: selcols=['ip', 'day', 'hour']; QQ=4; #'ip', 'day', 'hour'
        # if i==3: selcols=['ip', 'app']; QQ=4;
        if i==4: selcols=['ip', 'app', 'os']; QQ=4; #'ip', 'app', 'os'
        if i==5: selcols=['ip', 'device']; QQ=4; # ip', 'device'
        if i==6: selcols=['app', 'channel']; QQ=4; # 'app', 'channel'
        if i==7: selcols=['ip', 'os']; QQ=5; #'ip', 'os'
        if i==8: selcols=['ip', 'device', 'os', 'app']; QQ=4; #'ip', 'device', 'os', 'app'
    ...
    if os.path.exists(filename):
        ...
        train_df['category'] = (train_df['ip'].astype(str) + "_" + train_df['app'].astype(str) + "_" + train_df['device'].astype(str) \
            + "_" + train_df['os'].astype(str) + "_" + train_df['channel'].astype(str)).apply(hash) % D ## adding "channel "
